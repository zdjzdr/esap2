...
func (c Xm) String() string {
	return fmt.Sprintf("%v-%v  %v\n", c.Year, c.Jidu, c.Name)
}

func ywcxm(user string, id int) {
	queryAndSendArr(user, id, "select 年,季,授权任务 from [改进项目记录_主表] where 验收='通过' and 年=2015", &Xm{})
}
...
//备件-库存查询
func bjkc(user string, id int, input string) {
	sql := fmt.Sprintf("select mDesc + '/' + lot,iqty from vlbq2 where lcid='m09' and isnull(iqty,0)>0 and (charindex('%s',mdesc)>0 or charindex('%s',lot)>0)", input, input)
...
func zxrb(user string, id int) {
	...
	queryAndSendArr(user, id, "select 车号,sum(pqty),sum(sqty) from wx_zxclzx group by 车号", xm)
...
func bzyb(user string, id int) {
	...
	queryAndSendArr(user, id, "select mDesc,sum(pqty)as pqty,sum(isnull(sqty,0)) from wx_zxclbz group by mDesc order by pqty desc", xm)
	queryAndSend(user, id, "select '--总产量--',sum(pqty)as pqty,sum(isnull(sqty,0)) from wx_zxclbz", xm)
}

//素电库存字段：规格，型号，数量，暂放数，暂放率
type sd struct {
	Spec  string
	Type  string
	Qty   float32
...
func (c sd) String() string {
	return fmt.Sprintf("%v-%v: \n%v / %v\n暂放率: %v%%\n\n", c.Spec, c.Type, c.Qty, c.Zqty, c.Zrate)
}
...
func (c pjCl) String() string {
	return fmt.Sprintf("%v\n产量(万):%v\n损耗:%v\n记录日期：%v\n\n", c.X产线, c.X产量, c.X损耗, c.Date.Format("2006-01-02"))
}

func pjyb(user string, id int) {
	queryAndSendArr(user, id, "select cdate,产线,sum(pqty)as pqty,sum(isnull(sqty,0)) as sqty from wx_zxclpj group by cdate,产线 order by 产线", &pjCl{})
}
...
//订单进度字段：单号，子单号，日期，数量，计划数，下达数，前道数，后道数，入库数，发票数，开票数
type ddJd struct {
	SoNo  string
	SoRN  string
	Ddate time.Time
...
//订单进度
func ddjd(user string, id int, input string) {
	dd := &ddJd{}
	sql := fmt.Sprintf("SELECT 单号,项,交期,数量,计划=ISNULL(计划,0),下达=ISNULL(下达,0),前道=ISNULL(前道,0),完工=ISNULL(完工,0),入库=ISNULL(入库,0),发货=ISNULL(发货,0),开票=ISNULL(开票,0) FROM vSDSOplus where charindex('%s',soNoRn)>0", input)
	//延迟一秒再处理
...
const (
	FTPPATH   = `E:\esDisk\hr\emp\` //相对本程序服务器的照片存放路径，ES需启用网盘功能
	ESDISK    = `ed\esys`           //ES网盘根目录，管理控制台
...
func zpcl(w *wechat.WxReq) {
	...
	if v, ok := empMap[w.FromUserName]; ok {
		...
		if (v.Rcid) == nil {
			...
			sqlsrv.Exec("delete from es_casepic where rcid=? and r=? and c=?", v.Rcid, r, c)
...
//销售订单字段：单号，日期，创建人，业务员，产品，数量，价格，备注
type sxDd struct {
	No     string
	Cdate  time.Time
	Cre    string
...
func (w *AgentXF) Gevent() {
	...
	case "click":
		...
		case "wsjl": //未删记录
			go xfwsjl(w.req.FromUserName, w.req.AgentID)
		}
	}
}
