...
func (w *WxController) Get() {
	w.ParseForm(w.Ctx.Request)
	if !wechat.Valid(w.Ctx.ResponseWriter, w.Ctx.Request) { //验证是否为微信请求
		return
	}
}
...
func (w *WxController) Post() {
	...
	//	w.ParseReq()
	w.ParseReq(w.Ctx.Request) //解析微信请求
	if w.Req != nil {
		fmt.Printf("ReqXml:%#v\n", w.Req)
		...
		case "image":
			...
			w.Data["xml"] = wechat.RespTexts(w.Req.ToUserName, w.Req.FromUserName, "请输入员工信息(格式 姓名,工号)：")
		case "event":
			//			w.GoEvent()
		}
...
func (w *WxController) GoText() {
	...
	if v, ok := empMap[w.Req.FromUserName]; ok {
		...
		if len(empInfo) == 2 {
			...
			w.Data["xml"] = wechat.RespTexts(w.Req.ToUserName, w.Req.FromUserName, w.RespStr)
			return
		}
		...
		case "蛋蛋":
			...
			//创建文章回复
			w.Data["xml"] = wechat.RespArts(w.Req.ToUserName, w.Req.FromUserName, art, art2, art3, art4)
		default:
			...
			//创建文本回复
			w.Data["xml"] = wechat.RespTexts(w.Req.ToUserName, w.Req.FromUserName, fmt.Sprintf("%v", *resp))
		}
	}
}
