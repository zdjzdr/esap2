...
var (
	token          = "esap"
	corpId         = "wx1d2f3335684660"
	encodingAesKey = "K3C7Y9wQhXEsRjCEici1mpXhC3WSCFELbCQ4Ss6xMM"

	port = ":80"
)

func init() {
	wechat.SetSecret("ul2YxIsveBkIkgTjvzXwrWQRqcVynIzDfsTEbmRINrNXQZ4KGAntPWZZxYvlq")
	wechat.SetBiz(token, corpId, encodingAesKey)
	go wechat.FetchCorpAccessToken2()
}

func MsgTest() {
	bd, _ := wechat.TextMsg("admin", "testmsg", 1)
	for {
...
func wxhander(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case "text":
			//回复文本
			bd, _ := wechat.TextMsg(wr.FromUserName, wr.Content, wr.AgentID)
			for i := 0; i < 3; i++ {
				go wechat.SendMsg(bd)
			}
