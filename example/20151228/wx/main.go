...
func (w *myApp) GoText() {
	...
	if v, ok := empMap[w.Req.FromUserName]; ok {
		...
		default:
			//词条更新接口
			if strings.HasPrefix(w.Req.Content, "#[esap]") {
				str := strings.TrimLeft(w.Req.Content, "#[esap]")
				strs := strings.Split(str, "|")
				err := sqlsrv.Exec("insert wxr1(keyword,resp) values(?,?)", strs[0], strs[1])
				resp := "添加成功"
				if err != nil {
					resp = "添加失败"
				}
				...
				//尝试匹配用户关键字
				resp := sqlsrv.Fetch(fmt.Sprintf("select resp from wxr1 a1 where charindex('%s',keyword)>0 and not Exists(select id from wxr1 where charindex('%s',keyword)>0 and id>a1.id )", w.Req.Content, w.Req.Content))
				//未匹配到则使用“默认”所匹配的信息
				if *resp == nil {
